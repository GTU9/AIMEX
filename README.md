...
인플루언서별로 학습된 **LoRA 어댑터**를 얹어, 같은 베이스 모델이라도 캐릭터마다 다른 말투·성격으로 대화합니다. 아래는 츤데레 캐릭터 "미라"가 반말 톤과 성격을 유지하면서도 자연스럽고 일관된 한국어로 답하는 예시입니다.
...
인플루언서에 등록한 문서에서 질문과 관련된 부분을 찾아 그 내용을 근거로 답합니다. 답변에는 **어떤 문서의 어느 부분(스니펫)을 참고했는지와 유사도(%)** 가 카드 형태로 함께 표시되어, 근거를 사용자가 직접 확인할 수 있습니다. 아래 예시에서는 *에어팟 프로* 매뉴얼을 근거로 배터리·방수 등급 같은 사실을 정확히 캐릭터 말투로 답합니다.
...
등록된 문서로 답할 수 없는 최신·외부 정보는 **웹검색 MCP**로 자동 보강합니다. 문서 근거가 임계값에 못 미치면 웹검색으로 폴백하므로, 지식 범위 밖 질문에도 끊기지 않고 답합니다. (예: "파이썬을 만든 사람" → 귀도 반 로섬)
...
인플루언서의 말투로 게시글(주제·내용·이미지·해시태그)을 작성하고 초안으로 저장합니다. 내용은 **AI 생성**으로 캐릭터 말투에 맞춰 자동 작성할 수 있습니다. 작성물은 게시글 목록에서 다시 볼 수 있습니다.

<img src="etc/screenshots/content-create.png" width="760" alt="콘텐츠 작성 - 게시글 초안" />
