...
인플루언서를 고르고 주제·이미지를 입력하면 게시글을 작성합니다. 생성은 **2단계로 분리**되어 있습니다 — ① **원본 본문**은 이미지+주제를 GPT 멀티모달로 작성(중립)하고, ② **"인플루언서 말투로 변환"** 단계에서 그 인플루언서의 **LoRA**가 원본을 캐릭터 말투로 다시 씁니다. 원하는 본문을 적용한 뒤 해시태그와 함께 초안으로 저장합니다. (아래 예시는 한세나 — "진짜 대박이네! 그 활력 넘치는 분위기는 마치 마법 같아!"처럼 같은 주제가 캐릭터 말투로 변환됨)
