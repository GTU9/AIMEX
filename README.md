...
인플루언서별로 학습된 **LoRA 어댑터**를 얹어, 같은 베이스 모델이라도 캐릭터마다 다른 말투·성격으로 대화합니다. 현재 미라(츤데레 비서)·징크스·케이틀린·알라라크 등 여러 캐릭터가 각자의 어댑터로 동작합니다. 아래는 "알라라크"(오만한 군주 콘셉트)가 정체성을 유지하며 답하는 예시입니다.
